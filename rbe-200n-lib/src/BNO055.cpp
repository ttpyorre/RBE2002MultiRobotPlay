...
#define DEGREETORADIAN 0.0174532925199 // pi/180
#define RADIANTODEGREE 57.2957795131 // 180/pi
...
vector<double> BNO055::calculateAcceleration(void)
{
  vector<int16_t> acc = readRawAcc();
  vector<double> accOut = {0, 0, 0};

  /*  
  * Getting the accelerations for accOut. 
  * We divide by 100, as that is in the datasheet.
  * A bit more accurate might be dividing by 1000/9.81 to get the value in m/s^2.
  */
  accOut[0] = acc[0]/100.0;
  accelerationX = accOut[0];
  
  accOut[1] = acc[1]/100.0;
  accelerationY = accOut[1];

  accOut[2] = acc[2]/100.0;
  accelerationZ = accOut[2];
  
  return accOut;
}

//Converts the gyro readings from the IMU and returns the angular velocity
vector<double> BNO055 :: gyroToAngularVec(void)
{
  vector<int16_t> gyro = readRawGyro();
  vector<double> gyroOut = {0, 0, 0};

  //The datasheets states 16 LSB/°/s overall resolution/sensitivity for the gyro.
  gyroOut[0] = gyro[0]* DEGREETORADIAN / 16.0;
  gyroOut[1] = gyro[1]* DEGREETORADIAN / 16.0;
  gyroOut[2] = gyro[2]* DEGREETORADIAN / 16.0;

  return gyroOut;
}

double BNO055::getPitchAcc(void)
{ 
  pitch = atan2(accelerationX, sqrt((accelerationY * accelerationY) + (accelerationZ * accelerationZ))) * RADIANTODEGREE;
