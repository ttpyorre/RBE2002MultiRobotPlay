...
bool IRDirectionFinder::available()
{
  ...
  {                   //read only the data length fits.
    for (int i = 0; i < 16; i++) 
    {
...
bool IRDirectionFinder::CameraSensedSignal()
{
  //IR camera sensing, and then driving.
  if (available()) 
  {
    for (int i = 0; i < 4; i++) 
    {
      Point point = ReadPoint(i);
...
void IRDirectionFinder::TransmitterToggle()
{
